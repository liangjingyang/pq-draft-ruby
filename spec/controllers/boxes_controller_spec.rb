...
RSpec.describe BoxesController, type: :controller do
  ...
  describe "GET #index" do
    it "returns http success" do
      ...
      expect(JSON.parse(response.body)['boxes'][0]['name']).to eq('223的草稿箱')
    end
  ...
  describe "POST #show" do
    it "returns http success" do
      ...
      expect(response).to have_http_status(:success)
      expect(JSON.parse(response.body)['name']).to eq('223的草稿箱')
    end
  end
  ...
  describe "POST #update" do
    it "returns http success" do
      ...
      expect(response).to have_http_status(:success)
      expect(JSON.parse(response.body)['name']).to eq("123的草稿箱")
    end
  end
